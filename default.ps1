...
Task Soup2Nuts -depends RebuildSqlServerContainer, CompileDb, DeployDb, RunTsqltTests
...
Task DeployDb {
    ...
    Exec {
        ...
        & $temp_tools_dir/sqlpackage/sqlpackage /Action:Publish /SourceFile:"$db_publish_dacpac" /tcs:$db_connection_str /p:BlockOnPossibleDataLoss=true
    }
}

task RunTsqltTests -depends RebuildSqlServerContainer, CompileDb, DeployDb{
    Write-Host "******************* Now running tsqlt unit tests *********************"  -ForegroundColor Green

    $env:ConnectionStrings:SampleDatabase = $db_connection_str

    Push-Location $base_dir
    $test_projects = @((Get-ChildItem -Recurse -Filter "*TsqlTests.csproj").FullName) -join '~'

    foreach($test_project in $test_projects.Split("~"))
    {
        Write-Host "Executing tests on: $test_project"
        exec {
            & dotnet msbuild /t:restore $test_project /v:m 
            & dotnet test $test_project --logger "console;verbosity=detailed" -- xunit.parallelizeTestCollections=$false
        }
    }
    Pop-Location
 }
